...
class RegistrationTests(TestCase):
    ...
    def test_successful_registration_with_phone(self):
        response = self.client.post(
            self.url,
            {"phone": "+15550001111", "password": "StrongPass123!"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertTrue(User.objects.filter(phone="+15550001111").exists())

    def test_registration_requires_at_least_one_identifier(self):
    ...
    def test_duplicate_email_returns_identifier_taken(self):
    ...
    def test_duplicate_phone_returns_identifier_taken(self):
        User.objects.create_user(phone="+15550002222", password="StrongPass123!")
        response = self.client.post(
            self.url,
            {"phone": "+15550002222", "password": "AnotherPass123!"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_409_CONFLICT)
        self.assertEqual(response.data["error"]["code"], "IDENTIFIER_TAKEN")
    ...
    def test_weak_password_rejected(self):
        response = self.client.post(
            self.url,
            {"email": "weakpass@example.com", "password": "12345"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_password_is_hashed_after_registration(self):
        self.client.post(
            self.url,
            {"email": "hashed@example.com", "password": "StrongPass123!"},
            format="json",
        )
